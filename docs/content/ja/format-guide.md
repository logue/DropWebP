...
本プログラムで使用しているMozJPEG（モズジェイペグ）は、このJPEGを更に「**見た目の劣化を抑えつつ、もっと小さく**」するためにMozillaが開発した技術です。

#### JPEGの特徴

- **非可逆圧縮**：ファイルを小さくするために、人間の目には見えにくい情報を削るため、画質が少し劣化します（圧縮率が高いほど劣化が目立ちます）。
- **高い圧縮率**：写真のように色が多く複雑な画像を、非常に小さくできます。
- **MozJPEG**：従来のJPEGより、同じファイルサイズなら高画質に、同じ画質ならよりファイルサイズを小さくできます。
...
| 形式            | 主なメリット                                         | 向いている利用シーン                                   | ロッシー | ロスレス | アルファ | 圧縮率 | 負荷 |
| --------------- | ---------------------------------------------------- | ------------------------------------------------------ | -------- | -------- | -------- | ------ | ---- |
| PNG（Oxipng）   | 画質が劣化しない、背景が透明にできる。               | ロゴ、アイコン、図表。                                 | ❌️      | ✅️      | ✅️      | 中     | 高   |
| JPEG（MozJPEG） | 写真を非常に小さくできる。                           | Web上の一般的な写真。                                  | ✅️      | ❌️      | ❌️      | 高     | 低   |
| WebP            | 高圧縮で高画質、透過・アニメにも対応。               | Webサイトの汎用的な画像。                              | ✅️      | ✅️      | ✅️      | 中     | 中   |
| AVIF            | **最も高い圧縮率と高画質**、広色域対応。             | 高画質と高速表示を両立したいWebサイト。                | ✅️      | ❌️      | ✅️      | 高     | 高   |
| JPEG XL         | すべてに対応し、JPEGとも互換性がある**究極の形式**。 | 既存のJPEGを小さくしたいとき、将来的な画像形式の統一。 | ✅️      | ✅️      | ✅️      | 高     | 低   |
