...
public class WebPService : IWebPService
{
    ...
    public WebPService(ILocalSettingsService localSettingsService)
    {
    ...
    public bool ConvertWebP(string path, string outputPath, long quality = -1)
    {
        ...
        if (!localSettingsService.ReadSettingAsync<bool>("KeepOriginal").Result)
        {
            // オリジナルを保持しない場合
            File.Delete(path);
        }
    ...
    */
    ...
    private List<string> ImageFileExtensions()
    {
        ...
        if (localSettingsService.ReadSettingAsync<bool>("IgnoreJpeg").Result)
        {
            // Jpegを無視する場合
